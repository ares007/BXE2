...
if(this.cssr){A.selectEditableRange(this.cssr)
}else{if(A.anchorNode&&A.anchorNode.compareDocumentPosition(this.node)&Node.DOCUMENT_POSITION_CONTAINS){A.collapse(document.documentElement.firstChild,0)
...
for(i=0;
i<C.length;
i++){if(C[i].nodeType!=3&&!C[i].vdom.bxeDontshow&&!C[i].vdom.bxeNoaddappenddelete&&C[i].vdom.bxeTabletype!="table-row"&&C[i].vdom.bxeTabletype!="table-col"){if(i==0||C[i].vdom!=C[i-1].vdom){if(C[i].vdom.bxeHelptext){var F=C[i].vdom.bxeHelptext
}else{var F=false
...
I.init()
}}else{if(!(G.compareDocumentPosition(K.AppendToNode._node)&Node.DOCUMENT_POSITION_CONTAINED_BY)){var I=K.AppendToNode._node.appendChild(K.AppendToNode._node.ownerDocument.createTextNode(""));
I.init()
...
Widget_ModalBox.prototype.submitAndClose=function(H){if(H){var D=H.currentTarget.Widget;
H.preventDefault()
}else{var D=this
}var G=document.getElementById("__submit");
if(G){G.focus()
}D.hide();
bxe_registerKeyHandlers();
if(H){var C=H.currentTarget.elements;
var A=new Array();
for(var B=0;
B<C.length;
B++ in C){if(C[B].name.substr(0,2)!="__"){A[C[B].name.replace(/ +\*/,"")]=C[B].value
}}var F=window.getSelection();
try{if(F.anchorNode.nodeName=="HEAD"){F.collapse(document.body.firstChild,0)
}else{F.collapse(D.selAnchorNode,D.selAnchorOffset);
F.extend(D.selFocusNode,D.selFocusOffset)
}}catch(E){F.selectEditableRange(D.cssr)
...
I.selectEditableRange(J.target.Widget.cssr)
}if(J.target.Widget.callbackCancel){J.target.Widget.callbackCancel()
...
if(this.focusNode.nodeValue.substr(this.focusOffset,1)==" "){d=this.anchorOffset;
S=this.anchorNode
...
}if(node.parentNode.canHaveText){popup.addMenuItem(bxe_i18n.getText("Clean "),function(e){var widget=e.currentTarget.Widget;
var delNode=widget.MenuPopup.MainNode;
var par=delNode.parentNode;
try{var _prev=delNode._htmlnode.previousSibling;
var sel=window.getSelection();
...
E=A.iterateNext();
var D=new Array();
while(E){var F=E.getAttribute("__bxe_id");
if(E.hasAttribute("__bxe_attribute")){F+="/"+E.getAttribute("__bxe_attribute")
}if(!D[F]){C.push(E);
D[F]=true
}E=A.iterateNext()
}return C
...
}function bxe_Transform_first(){bxe_Transform();
var A=window.getSelection();
...
if(P>0){if(G.substringData(P-1,1)==STRING_NBSP){G.replaceData(P-1,1," ")
...
}catch(V){Q.parentNode.betterNormalize();
Q.insertData(P,A)
}}return true
}return false
...
}D=D&A[C].XMLNode.isNodeValid(true,null,B)
}return D
};
